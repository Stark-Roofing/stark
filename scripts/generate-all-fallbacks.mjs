 ...
 *   → Now also includes the homepage (/) since vite build alone produces
 *     a content-empty index.html.
 *   → For URLs already prerendered (Puppeteer succeeded), files are skipped
 *     intact. Only fills gaps.
 *
 * Note: this is a SEO safety net. The React SPA still hydrates client-side
 * for real users; this fallback HTML is what Googlebot sees if Puppeteer
 * prerender failed. Do not duplicate brand voice/design tweaks here.
 */
...
// Common business data — kept synced with the React app's branding
const BIZ = {
...
// Map URL pathname → page-specific SEO metadata
// Keep titles ≤ 70 chars, meta descriptions ≤ 160 chars
const URL_METADATA = {
  '/': {
...
// Service pages template generator
...
function metadataForPath(pathname) {
  ...
  // Legacy short city URLs (redirect, but fallback handles request anyway)
...
// Includes <link rel="modulepreload"> tags Vite injects when build splits
// vendor chunks — without these, the entry script fails to resolve imports
// and the React app shows a blank screen.
function getAssetTags() {
  ...
  // Module preload tags must go in <head> alongside the CSS links
  return { cssLinks: [...cssLinks, ...modulePreloads], scripts };
}

// Generate full SEO-friendly HTML for a path
...
<body>
  ...
  <noscript>
    <!--
      Static SEO fallback — generated by scripts/generate-all-fallbacks.mjs.
      Wrapped in <noscript> so it ONLY renders for crawlers / users without JS.
      React users never see this content; the SPA hydrates inside #root above.
      Googlebot reads <noscript> content for indexing per Google docs.
    -->
    <header>
...
for (const pathname of unique) {
  ...
  // Old behavior: skip (assumed vite build's index.html was sufficient — NOT TRUE for SEO).
  // New behavior: if dist/index.html came from `vite build` only (Puppeteer prerender failed),
  // it has the head metadata but body is empty. We replace it with rich SEO content.
  // We detect the empty-body case heuristically: if the file's body lacks an <h1> tag,
  // it's the bare Vite shell.
  ...
  if (existsSync(file)) {
    // If existing file has an <h1>, prerender succeeded → keep it.
    // If not, it's an old empty fallback → replace.
    const existing = readFileSync(file, 'utf8');
    if (/<h1[\s>]/i.test(existing)) {
      skipped++;
      continue;
    }
    log(`✓ ${pathname} (replaced empty fallback)`);
  }
  ...
  created++;
  if (!log.toString().includes(pathname)) log(`✓ ${pathname}`);
}
